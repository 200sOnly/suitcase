...
```sh
npm install --save the-suitcase
```

Or bower

```sh
bower install --save the-suitcase
```

Or from a CDN like unpkg.com

```html
<script type="text/javascript" src="https://unpkg.com/the-suitcase"></script>
```
...
You can instantiate the `the-suitcase` on its own using the `setupCache()` method and then attach the adapter manually to an instance of `axios`.

```js
// Import dependencies
import axios from 'axios'
import { setupCache } from 'the-suitcase'

// Create `the-suitcase` instance
...
You can use the `setup()` method to get an instance of `axios` pre-configured with the `the-suitcase`. This will remove `axios` as a direct dependency in your code.

```js
// Import dependencies
import { setup } from 'the-suitcase'

// Create `axios` instance with pre-configured `the-suitcase` attached to it
const api = setup({
  // `axios` options
...
After setting up `the-suitcase` with a specific cache configuration you can override parts of that configuration on individual requests.
...
You can allow `the-suitcase` to cache the results of a request using (almost) any HTTP method by modifying the `exclude.methods` list.
...
You can give a `localforage` instance to `the-suitcase` which will be used to store cache data instead of the default [in memory](https://github.com/200sOnly/suitcase/blob/main/src/memory.js) store.
...
// `async` wrapper to configure `localforage` and instantiate `axios` with `the-suitcase`
async function configure () {
  ...
  // Create `axios` instance with pre-configured `the-suitcase` using a `localforage` store
  return setup({
    // `axios` options
    baseURL: 'http://some-rest.api',
    ...
    cache: {
      maxAge: 15 * 60 * 1000,
      store: forageStore // Pass `localforage` store to `the-suitcase`
    }
  })
}
...
You can give a `RedisStore` instance to `the-suitcase` which will be used to store cache data instead of the default [in memory](https://github.com/200sOnly/suitcase/blob/main/src/memory.js) store.

_Note: This only works server-side_

```js
const { setup, RedisStore } = require('the-suitcase')
...
const api = setup({
  ...
  cache: {
    maxAge: 15 * 60 * 1000,
    store // Pass `RedisStore` store to `the-suitcase`
  }
})
...
You can give a `RedisDefaultStore` instance to `the-suitcase` which will be used to store cache data in Redis using the default commands instead of hash commands.

_Note: This only works server-side_

```js
const { setup, RedisDefaultStore } = require('the-suitcase')
const redis = require('redis')
...
const api = setup({
  ...
  cache: {
    maxAge: 15 * 60 * 1000,
    store // Pass `RedisDefaultStore` store to `the-suitcase`
  }
})
...
// Create `axios` instance with pre-configured `the-suitcase`
const api = setup({
  cache: {
    maxAge: 15 * 60 * 1000
  }
...
You can tell `the-suitcase` to read stale cache data when a network error occurs using the `readOnError` option.
...
By default `the-suitcase` clears stale cache data automatically, this would conflict with activating the `readOnError` option, so the `clearOnStale` option should be set to `false`.
...
You can customize how `the-suitcase` invalidates stored cache entries by providing a custom `invalidate` function.
...
A different version of `the-suitcase` is generated for node and the browser due to how Webpack 4 uses a `target` to change how the UMD wrapper is generated using `global` or `window`. If you are using the library in node or in your front-end code while using a module bundler (Webpack, rollup, etc) the correct version will be picked up automatically thanks to the `"main"` and `"browser"` fields in the `package.json`.

`the-suitcase` is developped in ES6+ and uses async/await syntax. It is transpiled to ES5 using `babel` with `preset-env`.
...
`the-suitcase` was designed to run in the browser. It does work in nodejs using the [in memory store](https://github.com/200sOnly/suitcase/blob/main/src/memory.js). But storing data in memory is not the greatests idea ever.
...
The better choice if you want to use `the-suitcase` server-side is to use a redis server with a `RedisStore` instance as explained above in the API section.
